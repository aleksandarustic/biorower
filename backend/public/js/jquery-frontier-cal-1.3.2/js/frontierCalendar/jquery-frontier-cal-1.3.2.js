...
(function($) {
	...
	function Calendar() {
		...
		*/
		...
		this.getAgendaItemsForDay = function(date){
			...
			//alert(JSON.stringify(itemsForDay));
		...
		this.renderSingleAgendaItem = function(agi){
			...
			//alert(agi.getAgendaData("idFile"));
		...
		this.renderAgendaDivElement = function(agi, displayMessage,startDayObject,endDayObject,leftEnd,rightEnd){
			...
			var nextY = this.getNextAgendaYPosition(startDayObject,endDayObject,agendaDivHeight,moreDivHeight) + 2;
			...
			if(nextY-2 > 0){
				...
				var mesg = $("<span/>");
				mesg.css("float","left");
				mesg.html("&nbsp;<img src='../images/rowing-icon.png' />&nbsp;"+displayMessage); //promena
				d.append(mesg);				
				if(rightEnd){
		...
		this.addMoreDivToDays = function(startDayObj,endDayObj,moreDivHeight){
			...
			d.html("<span class='btnMore'> + more (" + items.length + ") </span>");
			d.bind(
				'click',
				{
				...
				function(eventObj){
					...
					var agendaItems = eventObj.data.agendaItems;
					var date = eventObj.data.calDayDate;

					agendaItems.sort(Calendar.sortAgendaItemsByStartDate);

					//alert(JSON.stringify(agendaItems));
					...
					for(var i=0; i<agendaItems.length; i++){
						...
						agendaDiv.addClass("JFrontierCal-idItem-" + agi.getAgendaData("idFile"));
						...
						if(isEnd){
						 ...
						 agendaDiv.append(agi.getTitle() + "");
						if(agi.isAllDay()){
							// don't show start time
							agendaDiv.append("(All day event)<br>");
						}else{
							//var slika = $("#IDtempDevice-"+agi.getAgendaData("idDevice")).val();
							var slika = "../images/rowing-icon.png";
							agendaDiv.append("<div style=\" background-image: url("+ slika +"); height: 18px; width: 18px; border: 1px solid #c0c0c0; float:left; margin-right:5px; margin-top:4px; \"></div>");
							agendaDiv.append("<p style='float:left; margin-top:6px;'>"+DateUtil.getAgendaDisplayTime(agendaStartDt)+"</p>");
						...
						*/
						...
						agendaDiv.bind(
							'click', function() {
								...
								window.idFile = $(this).attr("class").split(" ")[1].split("-")[2];
							...
							*/
							...
							//this.clickAgendaFromCalendarMoreModalDialogHandler
						);						

						$("#modalMore").append(agendaDiv);

						// agendaDiv.append("From " + DateUtil.getAgendaDisplayTime(agendaStartDt) + " on " + agendaStartDt.toDateString() + 
						//	" To " + DateUtil.getAgendaDisplayTime(agendaEndDt) + " " + agendaEndDt.toDateString()  + "<br>")
					}
					/*
				    $("#myModalMore").find('#modal-more-dialog').addClass('modal-dialog-center').css({
				      'margin-top': function () {
				    ...
				    */
				    /*
					$('#myModalMore').on('shown.bs.modal', function() {
					        var initModalHeight = $('#modal-more-dialog').outerHeight();
					        $('#modal-more-dialog').css('margin-top', (window.screenY / 2) + initModalHeight);
					});
					*/
				    /*
			        $('#myModalMore').on('shown.bs.modal', function () {
	...
	Calendar.dayNames = new Array("MONDAY","TUESDAY","WEDNESDAY","THURSDAY","FRIDAY","SATURDAY", "SUNDAY");
	...
	*/
	...
	Calendar.showMoreAgendaModal = function(cal,date,agendaItems){
		...
		modal.attr("title", date.toDateString());
		...
		*/
		...
		for(var i=0; i<agendaItems.length; i++){
			...
			if(agi.isAllDay()){
				...
				var slika = $("#IDtempDevice-"+agi.getAgendaData("idDevice")).val();
				agendaDiv.append("<div style=\" background-image: url(/Images/device/"+ slika +"); height: 24px; width: 24px; border: 1px solid #c0c0c0; background-color:#fff; float:left; margin-right:5px; \"></div>");
				agendaDiv.append("<p style='float:left; margin-top:6px;'>"+DateUtil.getAgendaDisplayTime(agendaStartDt)+"</p>");
				/* agendaDiv.append("From " + DateUtil.getAgendaDisplayTime(agendaStartDt) + " on " + agendaStartDt.toDateString() +
				" To " + DateUtil.getAgendaDisplayTime(agendaEndDt) + " " + agendaEndDt.toDateString() + "<br>")*/
			}
	...
	DateUtil.getAgendaDisplayTime = function(date){
		...
		*/
		...
		//var dateTime= System.DateTime.ParseExact("03/14/2012 01:00:00 PM", "MM/dd/yyyy hh:mm:ss tt", System.Globalization.CultureInfo.InvariantCulture);
		//alert(dateTime);

		//brakpoint! format datuma

		//$a = new Date(h + c + m + c + s + " " + ampm);

		/* konvertovanje 
			$tempTime = h + c + m + c + s + " " + ampm;
			var time = $tempTime;
